...
test('40. Ramp functionality: ADA & Utility modes, warnings, overlap logic, BOM output, and save/load integrity', async () => {
  const { calculateRamp } = await import('./src/engine/structuralCalc.js');
  const { generateBOM } = await import('./src/engine/bomGenerator.js');
  ...
  assert.strictEqual(validateProjectData(serialized), true, 'Project with ramp should validate successfully');
  
  const serializedDeck = serialized.sections.find(s => s.id === deckSec.id);
  assert.ok(serializedDeck.ramp, 'Serialized deck should have a ramp object');
  assert.strictEqual(serializedDeck.ramp.mode, 'ada', 'Serialized ramp should preserve ADA mode');
  assert.strictEqual(serializedDeck.ramp.direction, 's', 'Serialized ramp should preserve direction');

  useDeckStore.getState().resetDeck();
  useDeckStore.getState().loadProject(serialized.sections, serialized.materials);
  const loadedDeck = useDeckStore.getState().sections.find(s => s.id === deckSec.id);
